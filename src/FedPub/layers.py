...
class MaskedGCNConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        #####################################################
        self.lin = MaksedGCNLinear(
            in_channels, out_channels, bias=False, l1=l1, weight_initializer="glorot"
        )
        self.d_o, self.d_i = self.lin.weight.size()
        self.mask = torch.nn.Parameter(
            torch.ones((self.d_o, self.d_i), requires_grad=True, dtype=torch.float32)
        )
